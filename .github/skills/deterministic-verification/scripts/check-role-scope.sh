...
if [[ ${#violations[@]} -gt 0 ]]; then
  {
    ...
    printf '  - %s\n' "${violations[@]}"
    printf '%s\n' 'Engineer implements and tests assigned code slices only. It must not author .workflow/CONTEXT.md, .workflow/UBIQUITOUS-LANGUAGE.md, .workflow/prd/**, .workflow/plans/phases/**, or AGENTS.md.'
    printf '%s\n' 'What to do:'
