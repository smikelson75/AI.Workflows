...
- Give every slice exactly one kind: `behavior`, `verification-only`, or `refactor`. A `behavior` slice is one vertical behavior. The final slice of every phase is a `verification-only` integration and/or end-to-end slice proving behavior implemented by earlier slices; it must not add or fix production behavior.
- Every slice states every verification command needed to prove it. Add an integration verification command to any slice that adds or changes a dependency boundary (network, database, filesystem, queue, external service, or process boundary); a slice that only changes in-process logic needs only its focused test command. Decide this at planning time so the orchestrator never has to infer it.
- A `behavior` slice uses TDD regardless of whether its nearest observable boundary requires a unit, integration, contract, process, or end-to-end test. A `verification-only` slice may add tests that pass initially, but excludes production behavior changes and production files from scope. A `refactor` slice changes structure without behavior and states baseline and final verification commands.
- Add a mutation-testing run to a slice's verification commands only when the user has enabled mutation testing and agreed a cadence; see [`mutation-testing/references/PROTOCOL.md`](../mutation-testing/references/PROTOCOL.md). Route survivors and any required prerequisite test-writing phase per that protocol.
- Do not choose a test framework, assertion library, or mocking library in a slice. State the verification command the repository actually uses; the test stack is discovered or explicitly settled by [`tdd`](../tdd/SKILL.md), not prescribed here. When no test project exists but `AGENTS.md` records a settled stack, the first behavior slice may include the minimum test project and dependencies needed for its first failing test and use the recorded focused/full-suite commands. If the stack is neither evidenced nor recorded, stop and route to `/tdd` in stack-settlement mode, then `/agent-instructions`; this is a setup prerequisite, not a reason to invent or rescope a phase.
- Write slice documents for the `in progress` phase. `Orchestrator` may expand the next phase's slices from an existing phase document at a phase boundary; keep every planned phase document complete enough for that to be elaboration rather than new planning.
